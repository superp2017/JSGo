...
func (pay *PayHandler) Init_wx_pay(env string) {
	...
	if config == nil {
		log.Fatalln("Init_wx_pay JsConfig.GetConfig() is nil\n ")
		return
	}
	...
	pay.WxPubPayUrl = PayType.WxPubPayUrl
	pay.WxPubRefundUrl = PayType.WxPubRefundUrl
	pay.WxPubTransferUrl = PayType.WxPubTransferUrl
	...
	if pay.C == nil {
		log.Fatalln("Init_wx_pay:pay.C is nil")
	}
	// 附着商户证书
...
func (pay *PayHandler) Wx_pay(order *Order) (map[string]string, error) {

	if pay == nil || pay.C == nil {
		return nil, errors.New("Pay or Pay.C is nil \n")
	}
	...
	if ret["return_code"] == "FAIL" {
		return nil, JsLogger.ErrorLog("微信提交订单失败,return_msg:%s,err_code_des:%s\n", ret["return_msg"], ret["err_code_des"])
	}
...
func (pay *PayHandler) Wx_refund(order *Order) (map[string]string, error) {
